...
public class ColleagueUpdateStudentAction implements OnlineSectioningAction<ColleagueUpdateStudentAction.UpdateResult> {
	...
	public ColleagueUpdateStudentAction withName(String firstName, String middleName, String lastName) {
		iFName = firstName;
		iMName = middleName;
		iLName = lastName;
		return this;
	...
	public ColleagueUpdateStudentAction withEmail(String email) {
		iEmail = email;
		return this;
	}
	...
	public ColleagueUpdateStudentAction withColleagueSectionId(Integer colleagueId) {
		iColleagueIds.add(colleagueId);
		return this;
	}
	
	public ColleagueUpdateStudentAction withOverride(String type, String subject, String course, String colleagueId) {
		iOverrides.add(new String[] {type, subject, course, colleagueId});
		return this;
	}
	...
	protected Map<InstructionalOffering, Map<String, Set<Class_>>> getOverrides(OnlineSectioningHelper helper) {
		...
		for (String[] override: iOverrides) {
			...
			Integer colleagueId = null;
			if (override[3] != null && !override[3].isEmpty()) {
				try {
					colleagueId = Integer.valueOf(override[3]);
				} catch (NumberFormatException e) {
					helper.warn("Failed to parse Colleague Id " + override[3]);
				}
			}
			...
			if (colleagueId == null) {
				...
				CourseOffering co = ColleagueSection.findCourseOfferingForColleagueIdAndTermCode(helper.getHibSession(), colleagueId, iTermCode);
				if (co == null) {
					helper.error("No course offering found for Colleague Id " + colleagueId + " and colleague session " + iTermCode);
					iResult = UpdateResult.PROBLEM;
					continue;
				}
				...
				for(Iterator<?> it = ColleagueSection.findAllClassesForColleagueIdAndTermCode(helper.getHibSession(), colleagueId, iTermCode).iterator(); it.hasNext();) {
				...
				if (!foundClass) {
					helper.error("No classes found for Colleague Id " + colleagueId + " and colleague session " + iTermCode);
					iResult = UpdateResult.PROBLEM;
					continue;
				}
			}
	...
	public Map<CourseOffering, List<Class_>> getEnrollments(OnlineSectioningHelper helper) {
		...
		for (Integer colleagueId: iColleagueIds) {
			CourseOffering co = ColleagueSection.findCourseOfferingForColleagueIdAndTermCode(helper.getHibSession(), colleagueId, iTermCode);
			if (co == null) {
				helper.error("No course offering found for Colleague Id " + colleagueId + " and colleague session " + iTermCode);
				iResult = UpdateResult.PROBLEM;
