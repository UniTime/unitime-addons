...
public class BannerOfferingsPage extends Composite {
	private static final GwtMessages MESSAGES = GWT.create(GwtMessages.class);
	private static final CourseMessages COURSE = GWT.create(CourseMessages.class);
	private static final BannerMessages BMSG = GWT.create(BannerMessages.class);
	protected static GwtRpcServiceAsync RPC = GWT.create(GwtRpcService.class);
	private SimplePanel iRootPanel;
	private SimpleForm iPanel;
	private PageFilter iFilter;
	
	private BannerOfferingsFilterResponse iConfig;

	public BannerOfferingsPage() {
		iFilter = new PageFilter();
		iFilter.getHeader().setCollapsible(!"0".equals(ToolBox.getSessionCookie("BannerOfferings.Filter")));
		iFilter.getHeader().addCollapsibleHandler(new ValueChangeHandler<Boolean>() {
			@Override
			public void onValueChange(ValueChangeEvent<Boolean> event) {
				ToolBox.setSessionCookie("BannerOfferings.Filter", event.getValue() ? "1" : "0");
			}
		});
		...
		iFilter.getFooter().addButton("search", BMSG.actionSearchBannerOfferings(), new ClickHandler() {
			@Override
		...
		iFilter.getFooter().getButton("search").setAccessKey(BMSG.accessSearchBannerOfferings().charAt(0));
		iFilter.getFooter().getButton("search").setTitle(BMSG.titleSearchBannerOfferings(BMSG.accessSearchBannerOfferings()));
	...
	protected void init() {
		RPC.execute(new BannerOfferingsFilterRequest(), new AsyncCallback<BannerOfferingsFilterResponse>() {
			@Override
			...
			@Override
			public void onSuccess(BannerOfferingsFilterResponse result) {
				iConfig = result;
	...
	protected void search(final AsyncCallback<Boolean> callback, boolean openDetailsOnSingleResult) {
		final BannerOfferingsRequest request = new BannerOfferingsRequest();
	...
	public static class BannerOfferingsFilterRequest implements GwtRpcRequest<BannerOfferingsFilterResponse> {}
	
	public static class BannerOfferingsFilterResponse extends FilterInterface {
	...
	public static class BannerOfferingsRequest extends OfferingsRequest {}
}
